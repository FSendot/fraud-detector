#!/usr/bin/env python3
"""Train the boosted-tree fraud branch on canonical tabular inputs."""
...
from models.boosted_branch import BoostedBranchConfig, feature_importance_frame, fit_boosted_branch  # noqa: E402
from training.train_utils import (  # noqa: E402
    binary_classification_metrics,
    classifier_prediction_frame,
    default_boosted_branch_paths,
    default_boosted_branch_prediction_paths,
    ensure_parent,
    load_model_input,
    set_global_seed,
    split_model_input,
...
def _load_config(path: Path | None) -> BoostedBranchConfig:
    base = BoostedBranchConfig()
    if path is None:
        return base
    with path.open("r", encoding="utf-8") as handle:
        payload = json.load(handle)
    return BoostedBranchConfig(
        ...
        max_leaf_nodes=int(payload.get("max_leaf_nodes", base.max_leaf_nodes)),
        ...
        l2_regularization=float(payload.get("l2_regularization", base.l2_regularization)),
        max_bins=int(payload.get("max_bins", base.max_bins)),
        early_stopping=bool(payload.get("early_stopping", base.early_stopping)),
        validation_fraction=float(payload.get("validation_fraction", base.validation_fraction)),
        n_iter_no_change=int(payload.get("n_iter_no_change", base.n_iter_no_change)),
        feature_importance_max_rows=int(payload.get("feature_importance_max_rows", base.feature_importance_max_rows)),
        feature_importance_repeats=int(payload.get("feature_importance_repeats", base.feature_importance_repeats)),
        feature_importance_scoring=str(payload.get("feature_importance_scoring", base.feature_importance_scoring)),
        feature_importance_n_jobs=int(payload.get("feature_importance_n_jobs", base.feature_importance_n_jobs)),
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train the boosted-tree fraud branch on canonical tabular model inputs.",
    )
...
def main() -> int:
    ...
    default_train, default_valid, default_test, default_contract, default_model, default_saved_config, default_metrics = default_boosted_branch_paths()
    default_valid_pred, default_test_pred, default_feature_importances = default_boosted_branch_prediction_paths()
    ...
    model = fit_boosted_branch(train_features, train_labels.astype(int), config=config)
    ...
    print(
        ...
        f"iterations={int(model.n_iter_)} "
